	...
	*/
	...
	public:
		...
		*/
		...
		//pyh remove resize Copy data
		void DirectSetFrom(const MemoryBlock<T> *source, MemoryCopyDirection memoryCopyDirection, uint noTotalTriangles)
		{
			switch (memoryCopyDirection)
			{
			case CPU_TO_CPU:
				memcpy(this->data_cpu, source->data_cpu,  noTotalTriangles * sizeof(T));
				break;
#ifndef COMPILE_WITHOUT_CUDA
			case CPU_TO_CUDA:
				ORcudaSafeCall(cudaMemcpyAsync(this->data_cuda, source->data_cpu, noTotalTriangles * sizeof(T), cudaMemcpyHostToDevice));
				break;
			case CUDA_TO_CPU:
				ORcudaSafeCall(cudaMemcpy(this->data_cpu, source->data_cuda, noTotalTriangles * sizeof(T), cudaMemcpyDeviceToHost));
				break;
			case CUDA_TO_CUDA:
				ORcudaSafeCall(cudaMemcpyAsync(this->data_cuda, source->data_cuda, noTotalTriangles * sizeof(T), cudaMemcpyDeviceToDevice));
				break;
#endif
			default: break;
			}

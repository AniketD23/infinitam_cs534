	...
	const ITMTrackingState *trackingState, const ITMRenderState *renderState)
{
	...
	for (int entryId = 0; entryId < noVisibleEntries; entryId++)
	{
		...
		for (int z = 0; z < SDF_BLOCK_SIZE; z++) for (int y = 0; y < SDF_BLOCK_SIZE; y++) for (int x = 0; x < SDF_BLOCK_SIZE; x++)
		{
			...
			float eta = computeUpdatedVoxelDepthInfo(localVoxelBlock[locId], pt_model, M_d, projParams_d, mu, maxW,
				depth, depthImgSize);
		...
		for (int targetIdx = 0; targetIdx < noTotalEntries; targetIdx++)
		{
			...
			case 1: //needs allocation, fits in the ordered list
				...
				if (vbaIdx >= 0) //there is room in the voxel block array
				{
					...
					//pyh enable vb hash tracking on CPU
					hashEntry.fused_counter = 0;
					hashEntry.init=true;

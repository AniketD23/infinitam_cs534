...
//pyh mainly adding flag to determine whether you want incremental mesh extractino or full mesh extraction
void ITMMeshingEngine_CPU<TVoxel, ITMVoxelBlockHash>::MeshScene(ITMMesh *mesh, const ITMScene<TVoxel, ITMVoxelBlockHash> *scene, int mesh_type)
{
    ...
    for (int entryId = 0; entryId < noTotalEntries; entryId++)
	{
		...
		for (int z = 0; z < SDF_BLOCK_SIZE; z++) for (int y = 0; y < SDF_BLOCK_SIZE; y++) for (int x = 0; x < SDF_BLOCK_SIZE; x++)
		{
			...
			for (int i = 0; triangleTable[cubeIndex][i] != -1; i += 3)
			{
				...
				triangles[noTriangles].clr0 = colorList[triangleTable[cubeIndex][i]].toIntRound();
				triangles[noTriangles].clr1 = colorList[triangleTable[cubeIndex][i + 1]].toIntRound();
				triangles[noTriangles].clr2 = colorList[triangleTable[cubeIndex][i + 2]].toIntRound();

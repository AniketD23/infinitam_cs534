...
namespace ITMLib
{
	...
	{
	public:
		//pyh marching cube extraction format 
		struct Triangle { Vector3f p0, p1, p2; Vector3i clr0, clr1, clr2; Vector3i vb_info; };
		...
		//pyh used to track how many voxelBlocks has been extracted 
		uint noVoxelBlocks;
		//pyh voxel blocks that needs to be updated
		std::vector<Eigen::Vector3f> updated_voxel_blocks;		
		...
		explicit ITMMesh(MemoryDeviceType memoryType, uint maxTriangles = noMaxTriangles_default)
		{
			...
			this->noVoxelBlocks=0;

			//triangles = new ORUtils::MemoryBlock<Triangle>(noMaxTriangles_default, memoryType);
			//pyh we are not using this
			triangles = new ORUtils::MemoryBlock<Triangle>(0, memoryType);
		}
